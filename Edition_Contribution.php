...
function Aff_Pers2($suffixe, $donnees_ini, $donnees_prop)
{
	...
	if ($presence_prop == 'O') {
		echo '<input type="checkbox" name="maj_' . $suffixe . '" value="O"/>Nom en majuscules<br /><br />';
		if (($suffixe == 'pere') or ($suffixe == 'mere')) {
			if ($donnees_ini['reference'] != 0) $action = 'Remplacer';
			else                                $action = 'Ajouter';
			echo '<input type="radio" name="' . $nom_rb . '" value="' . $action . '"/>' . $action . '<br />' . "\n";
		}

		if (($suffixe == 'conj') or ($suffixe == 'enfant1') or ($suffixe == 'enfant2')) {
			if ($suffixe == 'conj') $action = 'Remplacer conjoint ';
			else                    $action = 'Remplacer enfant ';
			// 1er choix : ajouter
