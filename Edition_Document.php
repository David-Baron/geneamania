...
$Reference = Recup_Variable('Reference', 'N');// Recup des variables passées dans l'URL : identifiant, référence et type de référence
...
function Aff_Formulaire()
{
	...
	if ($result->rowCount() == 0) {
		echo 'Vous n\'avez pas créé de type de document<br />' . "\n";
...
if ($bt_OK) {
	...
	if ($NomFic != '') {
		...
		// Type inconnu, erreur !
		if ($NatureDoc == '') $erreur = "Impossible de déterminer le type du fichier !";
		...
		if ($erreur == '') {
			...
			if (!ctrl_fichier_ko()) {
				...
				if (!move_uploaded_file($_FILES['nom_du_fichier']['tmp_name'], $nomComplet)) {
					$erreur = 'Impossible de placer le fichier dans le bon répertoire';
				}
