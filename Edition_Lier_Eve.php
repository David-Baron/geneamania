...
if (($bt_Sup) or ($bt_OK)) {
	...
	echo '<input type="' . $hidden . '" name="dDebAnc" value="' . $dDebLue . '"/>' . "\n";
	echo '<input type="' . $hidden . '" name="dFinAnc" value="' . $dFinLue . '"/>' . "\n";
	...
	if ($refPers == -1) {
		echo colonne_titre_tab($LG_Link_Ev_Pers_Pers);
	...
	if ($refEvt == -1) {
		echo colonne_titre_tab($LG_Link_Ev_Pers_Event_Type);
		...
		echo colonne_titre_tab($LG_Link_Ev_Pers_Event);
		...
		echo '<button type="submit" class="positive" ' .
			...
			'<img src="' . $chemin_images . $Icones['dernier_ajoute'] . '" alt=""/>' . my_html($LG_Link_Ev_Pers_Last_Event) . '</button>';
		echo '</div>';
		echo "</td></tr>\n";
	}
	...
	//
	//  ===== Role
	echo colonne_titre_tab($LG_Link_Ev_Pers_Role);
	$refRole = '';
	...
	//  ===== Zone géographique
	echo colonne_titre_tab($LG_Link_Ev_Pers_Place);
	...
	while ($enr_zone = $result->fetch(PDO::FETCH_NUM)) {
		echo '<input type="radio" name="idNiveauF" value="' . $enr_zone[0] . '"';
		if ($enr_zone[0] == $idNiveauLu) echo ' checked="checked"';
		echo ' onclick="bascule_image(\'img_zone\')"/>' . my_html($enr_zone[1]) . ' ' . "\n";
	}
	...
	echo colonne_titre_tab($LG_Link_Ev_Pers_Beg_Part);
	echo '<input type="text" readonly="readonly" name="dDebAff" value="' . Etend_Date($dDebLue) . '"/>' . "\n";
	Affiche_Calendrier('imgCalendD', 'dDebPart()');
	echo '<input type="' . $hidden . '" name="dDebCache" value="' . $dDebLue . '"/>' . "\n";
	echo "</td></tr>\n";
	//
	//  ===== Date de fin de participation
	echo colonne_titre_tab($LG_Link_Ev_Pers_End_Part);
	echo '<input type="text" readonly="readonly" name="dFinAff" value="' . Etend_Date($dFinLue) . '"/>' . "\n";
	Affiche_Calendrier('imgCalendF', 'dFinPart()');
	echo "<input type='hidden' name='dFinCache' value='$dFinLue'/>\n";
	echo '   <img src="' . $chemin_images . $Icones['copie_calend'] . '" alt= "' . $LG_Link_Ev_Pers_Paste_Beg . '" title="' . $LG_Link_Ev_Pers_Paste_Beg .
		'"onclick="copieDate();"/>' . "\n";
	echo "</td></tr>\n";

	//  ===== Etiquette GénéGraphe, uniquement en local
	if ($Environnement == 'L') {
		echo colonne_titre_tab($LG_Link_Ev_Pers_Save_GeneGraphe);
