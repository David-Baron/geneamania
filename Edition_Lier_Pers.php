...
if ($clic_boutons) {
	...
	if ($bt_OK) {
		...
		if ($Creation) {
			...
			if (!$existe) {
				$message .= 'Création d\'un nouveau lien<br />';
				...
				$result = maj_sql($requete);
				if ($result == 1) $message .= 'Création correcte<p>';
			} else $message .= 'Création impossible car lien déjà présent<p>';
			$retourPreced = true;
			maj_date_site();
		}
		//  Mise a jour
			...
			$message .= 'Mise à jour d\'un lien<br />';
			...
			if ($valeurs != '') {
				...
				$result = maj_sql($requete);
				if ($result == 1) {
					$message .= 'Mise à jour correcte<p>';
					$retourPreced = true;
					maj_date_site();
				}
	...
	//include('jscripts/Edition_Lier_Pers.js');
	...
	if ($Creation) {
		...
		echo '<button type="submit" class="positive" ' .
			...
			'<img src="' . $chemin_images . $Icones['dernier_ajoute'] . '" alt=""/>' . my_html($LG_Link_Pers_Last_Pers) . '</button>';
		echo '</div>';
		echo "</td></tr>\n";
	}
	...
	echo '<input type="radio" name="PrincipalF" value="O"';
	if ($PrincLue == 'O') echo ' checked="checked"';
	echo '/>' . my_html($LG_Yes) . ' ' . "\n";
	echo '<input type="radio" name="PrincipalF" value="N"';
	if ($PrincLue == 'N') echo ' checked="checked"';
	echo '/>' . my_html($LG_No) . ' ' . "\n";
	echo '<input type="radio" name="PrincipalF" value="I"';
	if (($PrincLue == 'I') or ($Creation)) echo ' checked="checked"';
	echo '/>' . my_html($LG_Link_Pers_No_Matter) . "\n";
