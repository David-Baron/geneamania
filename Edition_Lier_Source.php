...
if ((!$bt_OK) && (!$bt_An) && (!$bt_Sup)) {
	...
	if ($refSrc == -1) {
		...
		$sql = 'SELECT Ident, Titre FROM ' . nom_table('sources') .
			' WHERE Ident NOT IN ( SELECT Id_Source FROM ' . nom_table('concerne_source') .
		...
		if ($nb_sources > 0) {
			...
			$sql = 'SELECT MAX(Ident) FROM ' . nom_table('sources');
			$resMax = lect_sql($sql);
