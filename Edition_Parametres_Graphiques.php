...
{
	global $chemin_images, $chemin_images_arbres, $Icones, $aff_infos, $LG_Graphics_First, $LG_Graphics_Stop, $LG_Graphics_Next, $LG_Graphics_Last, $w_fonds, $h_fonds, $w_barres, $h_barres, $w_lettres, $h_lettres, $w_arbres, $h_arbres, $Image_Fond, $Lettre_B, $Image_Barre, $Image_Arbre_Asc;
	...
	if ($cancel) {
		...
		echo '  <img id="im_cancel_' . $extension . '" src="' . $chemin_images . $Icones['cancel'] . '" alt="' . $texte_im . '" title="' . $texte_im . '" onclick="efface(\'' . $extension . '\')"/>';
	}
	echo '</td>';
	$src = '';
	$texte_im = 'choix';
	switch ($extension) {
		...
		case 'barres':
			$larg = $w_barres;
			$haut = $h_barres;
			$src = $chemin_images . $Image_Barre;
			break;
		case 'lettres':
			$larg = $w_lettres;
			$haut = $h_lettres;
			$src = $chemin_images . $Lettre_B;
			break;
		case 'arbres_asc':
			$larg = $w_arbres;
			$haut = $h_arbres;
	...
	echo '<img id ="im_premier_' . $extension . '" src="' . $chemin_images . $Icones['first']
		. '" alt="' . $LG_Graphics_First . '" title="' . $LG_Graphics_First . '" onclick="premier(\'' . $extension . '\')"/> ';
	echo '<img id="im_precedent_' . $extension . '" src="' . $chemin_images . $Icones['stop']
	...
	echo '<img id="im_suivant_' . $extension . '" src="' . $chemin_images . $Icones['next']
		. '" alt="' . $LG_Graphics_Next . '" title="' . $LG_Graphics_Next . '" onclick="avance(\'' . $extension . '\',nb_cases_' . $extension . ')"/> ';
	echo '<img id="im_dernier_' . $extension . '" src="' . $chemin_images . $Icones['last']
...
{
	global $chemin_images, $Icones, $LG_Graphics_Init_Color;
	...
	echo '<td valign="middle">'
		. ' <img id="im_dernier_' . $nom . '" src="' . $chemin_images . $Icones['conversion']
...
function aff_degrade($couleur, $val_degrade, $lib_couleur)
{
	global $Degrade;
	echo '<tr>' . "\n";
	echo '<td width="15%"><input type="radio" name="SelDegrade" value="' . $val_degrade . '"';
...
function affiche_radio_prop($dominante)
{
	...
	echo '<input type="radio" name="choix" value="' . $dominante . '" onclick="
	...
	"/>' . my_html($gra_description[$dominante]) . ' ';
}
...
if ((!$bt_OK) && (!$bt_An)) {
	...
	echo '<img id="lettre" src="' . $chemin_images . $Lettre_B . '" alt="B" title="B" border="0" />' . my_html($LG_Graphics_Welcome) . ' ...';
	echo '</td></tr>';
	echo '</table>' . "\n";
	echo '</td>';
	echo '<td width="33%">';
	...
	echo '<table width="95%" border="0" class="classic" cellspacing="1" cellpadding="3" align="center">';
	echo '<tr align="center">';
	echo '<td id="barre" style="background-image:url(\'' . $chemin_images . $Image_Barre . '\'); background-repeat:repeat-x;">';
	...
	if ((!$SiteGratuit) or ($SiteGratuit and $Premium)) {
		echo '<br />Fond personnalisé : <input type="file" name="nom_du_fichier" id="nom_du_fichier" value="" onchange="readURL(this,\'image_copie_fonds\');"/>' . "\n";
	}
...
<script type="text/javascript">
	<?php
	echo 'var im_stop = "' . $chemin_images . $Icones['stop'] . '";' . "\n";
	echo 'var im_next = "' . $chemin_images . $Icones['next'] . '";' . "\n";
	echo 'var im_prev = "' . $chemin_images . $Icones['previous'] . '";' . "\n";
	...
	echo 'var chemin_images_lettres = "' . $chemin_images . 'lettres/";' . "\n";
	echo 'var nb_cases_barres = ' . $nb_cases_barres . ';' . "\n";
	echo 'var chemin_images_barres = "' . $chemin_images . 'fonds_barre/";' . "\n";
	...
	var images_barres = [<?php Liste_Noms_Images($chemin_images . 'fonds_barre'); ?>];
	var nb_images_barres = images_barres.length;

	// Liste des lettres possibles
	var images_lettres = [<?php Liste_Noms_Images($chemin_images . 'lettres'); ?>];
