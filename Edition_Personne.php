...
{
	global $Refer, $Pere_GP, $Mere_GP, $Rang_GP, $Comportement, $Icones, $Images, $chemin_images_util, $chemin_images, $Commentaire, $Diffusion_Commentaire_Internet, $Modif, $Existe_Filiation, $Existe_Union, $Existe_Enfants, $SexePers, $SiteGratuit, $Premium, $LG_Data_tab, $LG_File, $LG_Add_Name, $lib_OK, $lib_Annuler, $LG_at, $LG_with, $LG_tip, $LG_child, $LG_son, $LG_daughter, $LG_of, $LG_andof, $largP;
	...
	echo '<img id="ajout_nom" src="' . $chemin_images . $Icones['ajout'] . '" alt="' . LG_PERS_ADD_NAME . '" title="' . LG_PERS_ADD_NAME . '"' .
		'onclick="inverse_div(\'id_div_ajout_nom\');document.getElementById(\'nouveau_nom\').focus();"/>' . "\n";
	if (isset($_SESSION['Nom_Saisi'])) {
		echo ' <img id="Ireprend_nom" src="' . $chemin_images . $Icones['copier'] . '" alt="' . LG_PERS_SAME_NAME . '" title="' . LG_PERS_SAME_NAME . '"' .
			' onclick="reprend_nom();"/>' . "\n";
	}
	...
	if (isset($_SESSION['Nom_Saisi'])) {
		echo ' <img id="Ireprend_villeN" src="' . $chemin_images . $Icones['copier'] . '" alt="' . LG_PERS_SAME_BIRTH_TOWN . '" title="' . LG_PERS_SAME_BIRTH_TOWN . '"' .
	...
	if (isset($_SESSION['Nom_Saisi'])) {
		echo ' <img id="Ireprend_villeD" src="' . $chemin_images . $Icones['copier'] . '" alt="' . LG_PERS_SAME_DEATH_TOWN . '" title="' . LG_PERS_SAME_DEATH_TOWN . '"' .
	...
	echo '<img id="calc1" src="' . $chemin_images . $Icones['calculette'] . '" alt="' . LG_PERS_CALC_SOSA . '" title="' . LG_PERS_CALC_SOSA . '"' .
		' onclick="etend_num_sosa();document.getElementById(\'NumeroP\').focus();"/>' . "\n";
	// Bouton pour numéro 1 ==> de cujus
	echo '<img id="im_decujus" src="' . $chemin_images . $Icones['decujus'] . '" alt="' . LG_PERS_DECUJUS . '" title="' . LG_PERS_DECUJUS . '"' .
	...
	echo '<td>' . my_html(LG_CH_COMMENT_VISIBILITY) . ' <input type="checkbox" name="Diff_Internet_NoteP" value="O"';
	...
	if ($Modif) {
		...
		if (Get_Parents($Personne, $Pere, $Mere, $Rang)) {
			if (($Pere != 0) or ($Mere != 0)) {
				...
				switch ($SexePers) {
					case 'm':
						echo my_html(ucfirst($LG_son));
						break;
					case 'f':
						echo my_html(ucfirst($LG_daughter));
						break;
					default:
						echo my_html(ucfirst($LG_child));
						break;
				}
			}
...
<script type="text/javascript">
	...
	var e = document.getElementById("SexeF");
	e.setAttribute("disabled","disabled");
	e.setAttribute("title","Non modifiable, présence d\'une union");
	e = document.getElementById("SexeM");
	e.setAttribute("disabled","disabled");
	e.setAttribute("title","Non modifiable, présence d\'une union");';
	?>
