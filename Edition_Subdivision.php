...
$tab_variables = array(
	'ok', 'annuler', 'supprimer', 'Nom_SubDiv', 'ANom_SubDiv', 'Zone_Mere', 'AZone_Mere', 'Latitude', 'ALatitude', 'Longitude', 'ALongitude', 'Divers', 'ADivers', 'Diff_Internet_Note', 'ADiff_Internet_Note', 'Statut_Fiche', 'AStatut_Fiche'
);
...
DEFINE('TYPE_OBJET', 'S');
DEFINE('N_TABLE', nom_table('subdivisions'));
...
function Aff_Subdiv($enreg2)
{
	...
	echo '<td>' . my_html(LG_CH_COMMENT_VISIBILITY) . ' <input type="checkbox" name="Diff_Internet_Note" value="O"';
...
if ($bt_Sup) {
	if ($Environnement == 'L') {
		if (!est_utilisee(true)) {
			$req = 'DELETE FROM ' . nom_table('subdivisions') . ' WHERE Identifiant_zone = ' . $Ident;
			$res = maj_sql($req);
			maj_date_site();
			Retour_Ar();
		} else {
...
if ($bt_OK) {
	...
	// Cas de la création
		...
		if ($Nom_SubDiv != '') {
			...
			$nouv_ident = Nouvel_Identifiant('Identifiant_zone', N_TABLE);
		}
	}
