...
$Notaire_KU           = Secur_Variable_Post($Notaire_KU, 30, 'S');
$ANotaire_KU          = Secur_Variable_Post($ANotaire_KU, 30, 'S');
...
{
	global $chemin_images, $Icones, $Images, $chemin_images, $hidden, $max_enf_rapides, $Commentaire, $Diffusion_Commentaire_Internet, $lib_OK, $lib_Okay, $lib_Annuler, $lib_Supprimer, $LG_add, $def_enc, $Uni_Sexe, $Conjoint, $Conjoint_1, $Conjoint_2, $Sexe_1, $Sexe_2, $LG_birth, $LG_death
	...
	// Onglets
	echo '<ul class="tabs">' . "\n";
	echo '<li><a href="#" onclick="return showPane(\'pnlDonGen\', this)" id="tab1">' . my_html(LG_CH_DATA_TAB) . '</a></li>' . "\n";
	// Certains onglets ne sont disponibles qu'en modification
	if ($Ref_Union != -1) {
		echo '<li><a href="#" onclick="return showPane(\'pnlEnf\', this)">' . my_html(LG_UNION_CHILDREN) . '</a></li>' . "\n";
		echo '<li><a href="#" onclick="return showPane(\'pnlEvts\', this)">' . my_html(LG_UNION_EVENTS) . '</a></li>' . "\n";
		echo '<li><a href="#" onclick="return showPane(\'pnlDocs\', this)">' . my_html(LG_CH_DOCS) . '</a></li>' . "\n";
	}
	echo '<li><a href="#" onclick="return showPane(\'pnlFiche\', this)">' . my_html(LG_CH_FILE) . '</a></li>' . "\n";
	echo '</ul>' . "\n";
	...
	if ((!$depuis_pers) or ($Conjoint_2 == -1) or (($depuis_pers) and ($Conjoint_2 != $Personne))) {
		...
		if (($Conjoint_2 == -1) and (!$Uni_Sexe)) {
			echo " <input type=\"button\" onclick=\"sel_der('f')\" value=\"Dernière femme saisie\" name=\"DF\"/>";
		}
	} else {
	...
	echo ' ' . my_html(LG_UNION_CONTRACT_NOTARY) . ' <input type="text" size="30" name="Notaire_KU" value="' . $enreg2['Notaire_K'] . '"/></td></tr>' . "\n";
