...
if ($bt_OK) {
	...
	$nom_fic_svg = construit_fic($chemin_exports, 'Export_' . $type_export . $suffixe . '#', $ext);
			...
			if ($Initialisation) {
				...
				if (est_table('general', $tablename)) {
					...
					$lesDonnees = 'INSERT INTO ' . nom_table('general') .
						...
						",'arbre_asc_hor_carre.png', 'O', 'N', 'C', 'R', 'bar_off_marron.gif', current_timestamp" .
						",'#B8A165', '#B1A980', '#49453B', '#EFEFEF', '#FEFEFE',9999, null, 'Arial','#000000', true" .
						");";
				...
				if (est_table('utilisateurs', $tablename)) {
					...
					ecrire($fp, "$lesDonnees");
					$lesDonnees = $deb . "'Gestionnaire', 'gestionnaire', 'cb5a837679b389074f2cbd407574f31ef5b98ca6d9f4fe3bb8101f62e43b5379', 'G',null);";
					ecrire($fp, "$lesDonnees");
				...
				if (est_table('evenements', $tablename)) {
					...
					ecrire($fp, "$lesDonnees");
					$lesDonnees = $deb . '"sortie de la version 2019.06","20190601GL","20190601GL",current_timestamp,current_timestamp,"V");';
					ecrire($fp, "$lesDonnees");
				}
	...
	if (!$SiteGratuit) {
		echo '<input type="radio" name="type_export" value="Internet" checked="checked"/>' . my_html(LG_EXPORT_TARGET_INTERNET);
		echo '<input type="radio" name="type_export" value="SiteGratuit"/>' . my_html(LG_EXPORT_TARGET_HOSTED);
	}
	echo '<input type="radio" name="type_export" value="Sauvegarde"/';
	if ($SiteGratuit) echo ' checked="checked"';
	echo '>' . my_html(LG_EXPORT_TARGET_BACKUP) . "\n";
	if (file_exists('initialisation.txt')) {
		echo '<br /><input type="radio" name="type_export" value="Initialisation" onclick="document.forms.saisie.ut_suf[1].checked = true;"/>Initialisation' . "\n";
		echo '<input type="radio" name="type_export" value="SQLite" onclick="document.forms.saisie.ut_suf[1].checked = true;"/>SQLite' . "\n";
	}
	echo '</td>';
	echo '</tr>' . "\n";
	...
	if ((!$SiteGratuit) or ($SiteGratuit and $Premium)) {
		...
		echo '<input type="radio" name="ut_suf" value="O" checked="checked"/>' . my_html(LG_EXPORT_FILE_SUFFIXE_WITH) . ',' . "\n";
		echo '<input type="radio" name="ut_suf" value="N"/>' . my_html(LG_EXPORT_FILE_SUFFIXE_WITHOUT) . "\n";
		echo '</td>';
		echo '</tr>' . "\n";
	} else echo '<tr><td><input type="hidden" name="suffixe" value="">&nbsp;</td></tr>' . "\n";

	colonne_titre_tab(LG_EXPORT_OMIT_RECENT);
	echo '<input type="checkbox" name="s_dates_recentes"/>';
	echo '&nbsp;' . my_html(LG_EXPORT_DATE_THRES) . '&nbsp;:&nbsp;<input type="text" size="3" maxlength="3" name="pivot" value="' . $Lim_Diffu . '"/>&nbsp;' . my_html(LG_EXPORT_YEARS) . "\n";
	echo '</td>';
	echo '</tr>' . "\n";
