...
if ($res = lect_sql($sql)) {
	if ($enr_union = $res->fetch(PDO::FETCH_ASSOC)) {
		...
		if ($sortie_pdf) {
			...
			$pdf->Cell(0, 5, $enr_mari['Numero'] . ' x ' . $enr_femme['Numero'], 'LTR', 1, 'C');
		...
		if (Rech_Commentaire($enr_union['Conjoint_1'], 'P')) {
			HTML_ou_PDF($decal . LG_COUPLE_REPORT_COMMENT . ' : ' . $Commentaire . '<br />', $sortie);
		}
		...
		if (($Date_Mar != '') or ($Ville_Mar != 0)) {
			...
			if ($Ville_Mar != 0) {
				HTML_ou_PDF(' ' . LG_AT . ' ' . lib_ville($Ville_Mar), $sortie);
			}
		...
		if (($Date_K != '') or ($Ville_Notaire != 0)) {
			...
			if ($Notaire != '') HTML_ou_PDF(LG_COUPLE_REPORT_CONTRACT_NOTARY . $Notaire . ', notaire', $sortie);
			if ($Ville_Notaire != '') HTML_ou_PDF(' ' . LG_AT . ' ' . lib_ville($Ville_Notaire), $sortie);
			HTML_ou_PDF('<br />' . "\n", $sortie);
		}
		...
		while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
			...
			$Sexe = $enr_enf['Sexe'];
			$sur = my_html($enr_enf['Surnom']);
			if ($sur != '') $sur = Lib_sexe(', dit', $Sexe) . ' ' . $sur;
			HTML_ou_PDF('   ' . my_html($enr_enf['Prenoms']) . ' ' . my_html($enr_enf['Nom']) . $sur, $sortie);
			...
			if (($Date_Nai != '') or ($Ville_Nai != 0)) {
				...
				if ($Ville_Nai != 0) {
					...
					if ($Ville_Nai != $LaVille) {
						$Lib_Ville_Nai = lib_ville($Ville_Nai);
						$LaVille = $Ville_Nai;
			...
			if (($Date_Dec != '') or ($Ville_Dec != 0)) {
				...
				if ($Ville_Dec != 0) {
					...
					if ($Ville_Dec != $LaVille) {
						$Lib_Ville_Dec = lib_ville($Ville_Dec);
						$LaVille = $Ville_Dec;
