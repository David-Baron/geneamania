...
{
	global $chemin_images_util, $est_contributeur, $SiteGratuit, $Premium, $premier_enf, $lst_conj, $premier_lib_v, $h_LG_AT, $Commentaire, $Diffusion_Commentaire_Internet;
	if (($Mari) or ($Femme)) {
		$crit = '';
		if ($type_aff == 'E') {
			$premier_enf = true;
		...
		$sql = 'select Enfant, rang, Pere, Mere from ' . nom_table('filiations') . ' where ' . $crit . ' order by Rang';
		$resE = lect_sql($sql);
		...
		if ($nb_enreg > 0) {
				...
				if ($Enfant != $exclu) {
					...
					if ($_SESSION['estPrivilegie'] or ($enregEnf['Diff_Internet'] == 'O')) {
						...
						if (($Date_Nai != '') or ($Ville_Nai <> 0)) {
							...
							if ($Ville_Nai <> 0) {
								echo $h_LG_AT . ' ' . lib_ville_new($Ville_Nai, 'O', true);
								if ($premier_lib_v) {
									appelle_carte_osm();
						...
						if (($Date_Dec != '') or ($Ville_Dec <> 0)) {
							...
							if ($Ville_Dec <> 0) {
								echo $h_LG_AT . ' ' . lib_ville_new($Ville_Dec, 'O', true);
								if ($premier_lib_v) {
									appelle_carte_osm();
									if (($Commentaire != '') and (($_SESSION['estPrivilegie']) or ($Diffusion_Commentaire_Internet == 'O'))) {
										echo Div_Note($Commentaire);
			...
			if ($nb_enfants > 0) {
				...
				if ($type_aff == 'E') //and ((! $rangs_OK) or (! $dates_OK)))
					echo '<a href="Edition_Rangs.php?Pere=' . $Mari . '&amp;Mere=' . $Femme . '">' . Affiche_Icone('arrange', my_html(LG_FFAM_RANK_REORG)) . '</a>';
			}
		}
		unset($enregEnf);
		$resE->closeCursor();
...
if (!$est_privilegie and $enreg2['Diff_Internet'] != 'O') {
...
$compl .= '<a ' . Ins_Ref_Arbre($Refer) . '>' .
	'<img border="0" src="' . $chemin_images . $Icones['arbre_ascP'] . '" alt="Arbres" onmouseover="inverse_div(\'bonus\');"/>' .
	'</a> ' .
