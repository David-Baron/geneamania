...
function aff_rupt($libelle) {
	global $sortie, $sortie_pdf;
	if(!$sortie_pdf) echo '<fieldset><legend>'.my_html($libelle).'</legend>';
	else HTML_ou_PDF('<br />___ '.my_html($libelle).' ___'."\n",$sortie);
}
	...
	if ($crit != '') {
		...
		if ($resEnf = lect_sql($sql)) {
			...
			while ($enregEnf = $resEnf->fetch(PDO::FETCH_ASSOC)) {
		        ...
		        HTML_ou_PDF('<br />'.$num_enf.') <b>'.$enreg2['Prenoms'].' '.$enreg2['Nom'].$sur.'</b><br />'."\n",$sortie);
				...
				if ($Ville_Naissance != 0) {
					if ($Ville_Naissance != $memo_ville) {
						$nom_ville = lib_ville($Ville_Naissance);
				...
				if ($Ville_Deces != 0) {
					if ($Ville_Deces != $memo_ville) {
						$nom_ville = lib_ville($Ville_Deces);
				...
				if (($Date_Nai != $date_nd) or ($nom_ville_nai != '')) {
					$ligne_dates .= $decal.Lib_sexe('N&eacute;',$Sexe).' '.$Date_Nai.$nom_ville_nai;
				}
				if (($Date_Dec != $date_nd) or ($nom_ville_dec != '')) {
					if ($ligne_dates != '')
						...
						$ligne_dates .= $decal.'D';
					$ligne_dates .= Lib_sexe('&eacute;c&eacute;d&eacute;',$Sexe).' '.$Date_Dec.$nom_ville_dec;
				}
...
if($sortie_pdf) {
	...
	if ($gen == '')
		$cadre = 'LTBR';
	$pdf->Cell(0, 5, $enr_pers['Prenoms'].' '.$enr_pers['Nom'] , $cadre , 1, 'C');
	
	$pdf->SetFont($font_pdf,'',11);
	if ($gen != '')
		$pdf->Cell(0, 5, html_entity_decode($gen, ENT_QUOTES, $def_enc) , 'LBR' , 1, 'C');
...
if ($resUn = lect_sql($sql)) {
	...
	while ($enregUn = $resUn->fetch(PDO::FETCH_ASSOC)) {
        ...
        HTML_ou_PDF($num_conj.') <b>'.$enreg2['Prenoms'].' '.$enreg2['Nom'].$sur.'</b><br />'."\n",$sortie);
