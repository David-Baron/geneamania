...
if ($bt_An) Retour_Ar();
	...
	if (($SiteGratuit) and ($Premium)) {
		if ($est_contributeur)
			echo '<br /><a href="' . Get_Adr_Base_Ref() . 'Recherche_Ville_Heberges.php?Nom=' . $enreg['Nom_Ville'] . '">' . my_html($LG_ICSV_Town_Search_Cloud) . '</a>' . "\n";
	}
	//else '<br /><br />';
	echo '<br />' . "\n";
	...
	if ((!$SiteGratuit) or ($Premium)) {
		...
		if ($cp == '?') $cp = '';
		echo '<a href="' . $adr_rech_ville . '?Ville=' . $n_ville_uc . '&CP=' . $cp . '" target="_blank">' . my_html(LG_ICSV_TOWN_SEARCH) . '</a>';
	}
