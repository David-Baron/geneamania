...
function traite_lieu($str)
{
	...
	if ($p_ville != -1) {
		...
		if ($c_lieux != $nb_format_lieux) {
			Affiche_Warning('La ligne (' . $nb_enr . ') ' . $str . ' ne comporte pas le nombre de zones prévues pour les zones géographiques');
		}
...
if ($ok == 'OK') {
	...
	$sec = date('s', $temps);
	$debut = $jour . '/' . $mois . '/' . $annee . ' à ' . $heure . 'h' . $minutes . ':' . $sec;
	...
	if ($nom_du_fichier != '') {
		...
		if (!$erreur) {
			...
			if ($fp = fopen($path, 'r')) {
				...
				if ($Maj) echo 'Demande de mise à jour des données<br />';
				...
				if (($Maj) and ($Init)) {
					...
					suppression('évènement', $n_evenements, 'm', 'Code_Type <> "AC3U"');
					...
					suppression('dépôt', $n_depots, 'm', 'Ident > 0');
            ...
            for ($nb_ev = 0;$nb_ev < count($types_evenements); $nb_ev++) {
              echo $types_evenements[$nb_ev].'/'.$titres_evenements[$nb_ev].'<br>';
				...
				while ($str = fgets($fp, 255)) {
						...
						if ($s_INDI) {
							...
							if (!$Maj) {
								echo '<font color="blue">Données personne : ' .
									...
									', date de décès : ' . Etend_date($date_deces) .
					...
					if (($s_HEAD) and ($niveau > 0)) {
						...
						if ($niveau == 2) {
							...
							if ($ss_DATE == 1) {
								if ($sous_section == 'TIME') $msg = '+' . $msg . ' à ' . $z2;
								else $msg = '';
							}
							if ($ss_GEDC == 1) {
							...
							if ($zone2 == 'NOTE') {
								...
								if ($niveau == 2) {
									...
									if ($arbo[$n1] == 'DEAT') {
										$pec_note = true;
										if ($comment != '') $comment .= '<br />';
										$comment .= 'Note sur le décès :<br />' . rtrim(substr($str, 7));
									}
								}
							}
				...
				echo 'Fichier impossible à ouvrir<br />';
			}
			fclose($fp);
			/*
        ...
        */
			...
			$minutes = date('i', $temps);
			$sec = date('s', $temps);
			$fin = $jour . '/' . $mois . '/' . $annee . ' à ' . $heure . 'h' . $minutes . ':' . $sec;
			...
			$tab = '  ';
			if ($Codage_ANSEL)
				echo 'Décodage des caractères ANSEL vers ANSI<br />';
			echo 'Résumé du traitement :<br />';
			echo $tab . 'Début : ' . $debut . '<br />';
			echo $tab . 'Fin : ' . $fin . '<br />';
			echo $tab . $nb_pers . ' personnes ' . $action . '<br />';
			if (isset($idNom)) echo $tab . $idNom . ' noms ' . $action_m . '<br />';
			...
			if ($maj_oui == 'on') {
				...
				$ic_conseil = '<img src="' . $chemin_images . $Icones['tip'] . '" alt="Conseil"/> ';
				echo $ic_conseil . my_html(LG_IMP_GED_REMIND_SOSA_1)
					. '<a href="' . $base_ref . 'Liste_Pers.php?Type_Liste=P">'
					. my_html(LG_IMP_GED_REMIND_SOSA_2) . '</a>. '
					. my_html(LG_IMP_GED_REMIND_SOSA_3)
					. '<a href="' . $base_ref . 'Verif_Sosa.php">'
					. my_html($LG_Menu_Title['Check_Sosa']) . '</a>,<br />' . "\n";
				echo $ic_conseil . my_html(LG_IMP_GED_REMIND_EVT)
					. '<a href="' . $base_ref . 'Fusion_Evenements.php">' . my_html($LG_Menu_Title['Event_Merging']) . '</a>.<br />';
				echo $ic_conseil . my_html(LG_IMP_GED_REMIND_INTERNET)
					. '<a href="' . $base_ref . 'Verif_Internet.php">' . my_html($LG_Menu_Title['Internet_Cheking']) . '</a>.<br />';
			}
		}
	}
