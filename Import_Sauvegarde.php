...
{
	global $db, $nb_req_table, $chemin_images, $Icones, $SiteGratuit, $Premium, $Pivot_Masquage, $depuis_heberge, $Environnement;
	if ($nb_req_table != -1) {
		echo $nb_req_table . ' ' . my_html(LG_IMP_BACKUP_READ_LINES) . '<br />';
...
if (($ok == 'OK') and ($loc_base == 'I')) {
	...
	$connexion = null;
	$db      = $base_int;                                 //echo 'db : '.$db.'<br />';
	$util    = $uti_int;                                  //echo 'util : '.$util.'<br />';
	$mdp     = $mdp_int;                                  //echo 'mdp : '.$mdp.'<br />';
	$serveur = $site_int . ':' . $port_int;                   //echo 'serv : '.$serveur.'<br />';
	try {
...
if ($bt_OK) {
	...
	if ($_FILES['nom_du_fichier']['name'] != '') echo $_FILES['nom_du_fichier']['name'];
	else echo $SelFic;
	echo '<br />';
	$erreur = '';
	$path = '';
	...
	if ($path != '') {
		if ($fp = fopen($path, $mode)) {
			...
			$nb_req_lues = 0;
			$nb_req_table = -1;  // Nombre de requêtes par table pour les sites gratuits ==> contrôle de cohérence
			$nb_req_ok = 0;
			...
			while (!feof($fp)) {
				...
				if (($num_ligne == 5) and ($depuis_heberge)) {
					if (strlen($ligne) > 10)
						$pref_fic = substr($ligne, 10);
					//echo 'pref_fic : '.$pref_fic.'<br />';
				...
				if ($ext == 'txt') {
					...
					if (($SiteGratuit) and ($car1 == '#')) {
						...
						if (strpos($ligne, 'version Génémania ') !== false) {
					...
					if ($car1 == '[') {
						...
						if (($table != 'general') or ($aff_pres_ut = "O")) {
							echo my_html(LG_IMP_BACKUP_TABLE_IN_PROGRESS) . ' ' . $table . ', ';
		...
		if ($nb_req_ok > 0) maj_date_site(true);
	}
}
