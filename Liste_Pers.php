...
function Aff_Ne_Dec($row) {
  ...
  if (($Ne != '') or ($Decede != '')) {
    ...
    //if ($Ne != '') HTML_ou_PDF(my_html('° ').Etend_date($Ne),$sortie);
    if ($Decede != '') {
	...
	if ($nb_lignes > 0) {
		...
		while ($row = $res->fetch(PDO::FETCH_NUM)) {
			...
			// Dans le cas de la catégorie, on affiche l'icone qui correspond
				...
				echo '<img src="'.$chemin_images.$Icones['tag_'.$row[3]].'" BORDER="0" alt="'.$NomObj.'" title="'.$NomObj.'"/>'.'&nbsp;';
			}
			...
			switch ($Type_Liste) {
				...
				case 'C' :
					...
					if ($est_gestionnaire) {
						if ($Type_Liste == 'N') echo '&nbsp;'.Affiche_Icone_Lien($deb_lien_crea.'N'.$params.'"','ajouter',my_html(LG_CREATE_PERS_BORN_IN).' '.$NomObj);
						if ($Type_Liste == 'D') echo '&nbsp;'.Affiche_Icone_Lien($deb_lien_crea.'D'.$params.'"','ajouter',my_html(LG_CREATE_PERS_DEAD_IN).' '.$NomObj);
					}
				...
				case 'M' :
				case 'K' :
					echo $NomA.'&nbsp;('.$row[0].')&nbsp;'.'<a href="Notaires_Ville.php?Ville='.$row[2].'&amp;Nom='.$le_nom.'">Notaires</a>&nbsp;';
					appelle_carte_osm();
...
// Sortie au format texte
    ...
    if($sortie_pdf) {
		...
		$pdf->Cell(0, 5, html_entity_decode($objet, ENT_QUOTES, $def_enc), 'LTRB' , 1, 'C');
  ...
  if ($res->rowCount() > 0) {
    ...
    while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
      ...
      switch ($Type_Liste) {
      	case 'C' :
        case 'P' : HTML_ou_PDF(my_html($row['Prenoms']),$sortie);
                   Aff_Ne_Dec($row);
                   break;
        case 'N' :
        case 'D' : HTML_ou_PDF(my_html($row['Nom'].' '.$row['Prenoms']),$sortie);
                   Aff_Ne_Dec($row);
                   break;
        case 'M' : HTML_ou_PDF(my_html($row['Nomf'].' '.$row['Prenomsf']),$sortie);
                   HTML_ou_PDF('&nbsp;x&nbsp;'.my_html($row['Nomm'].' '.$row['Prenomsm']),$sortie);
                   if ($row['Maries_Le'] != '') HTML_ou_PDF('&nbsp;(x '.Etend_date($row['Maries_Le']).')',$sortie);
                   break;
        case 'K' : HTML_ou_PDF(my_html($row['Nomf'].' '.$row['Prenomsf']),$sortie);
                   HTML_ou_PDF('&nbsp;x&nbsp;'.my_html($row['Nomm'].' '.$row['Prenomsm']),$sortie);
