...
function Affiche_Generation($Gener)
{
	...
	if (!$CSV) {
		...
		if ($Gener == 1) HTML_ou_PDF(my_html($LG_LPersG_first), $sortie);
		else HTML_ou_PDF(my_html($LG_LPersG_next), $sortie);
		HTML_ou_PDF(' ' . my_html($LG_LPersG_generation), $sortie);
	...
	if (!$omanquant) {
		...
		if (!$CSV) {
			...
			if (($est_privilegie) or ($Personne[6] == 'O')) {
				...
				if (!$texte)
					echo '<a ' . Ins_Ref_Pers($Personne[0]) . '>' . my_html($Personne[1] . ' ' . $Personne[2]) . '</a>';
				else
					HTML_ou_PDF(my_html($Personne[1] . ' ' . $Personne[2]), $sortie);
				HTML_ou_PDF('</td>' . "\n", $sortie);
				HTML_ou_PDF('<td width="20%">', $sortie);
				$ne = $Personne[4];
				if (($texte) and ($ne != '')) HTML_ou_PDF('&deg;&nbsp;', $sortie);
				//if (($texte) and ($ne != '')) HTML_ou_PDF(my_html('°').' ',$sortie);
				HTML_ou_PDF(Etend_Date($ne), $sortie);
				if ($lieux) {
					if ($Personne[7] != '') {
						if ($sortie == 'H')	echo '<br />' . my_html($LG_at . ' ' . $Personne[7]);
				...
				if ($lieux) {
					if ($Personne[8] != '') {
						if ($sortie == 'H')	echo '<br />' . my_html($LG_at . ' ' . $Personne[8]);
						else $pdf->WriteHTML(' ' . $LG_at . ' ' . $Personne[7]);
						if (!$texte) {
							$Lat_V = $Personne[9];
							$Long_V = $Personne[10];
				...
				HTML_ou_PDF('<td colspan="3">' . my_html($LG_Data_noavailable_profile) . '</td>' . "\n", $sortie);
			}
			HTML_ou_PDF('  </tr>' . "\n", $sortie);
		}
		// Sortie CSV
...
function Affiche_Personne_Absente($Personne, $nb, $nb_gen)
{
	...
	if (!$CSV) {
		...
		if (($est_privilegie) or ($Personne[6] == 'O')) {
			...
			$texte2 = my_html($texte2) . ' ';
			if (!$texte)
				echo $texte2 . '<a ' . Ins_Ref_Pers($Personne[0]) . '>' . my_html($Personne[1] . ' ' . $Personne[2]) . '</a>';
			else
				HTML_ou_PDF($texte2 . my_html($Personne[1] . ' ' . $Personne[2]), $sortie);
			HTML_ou_PDF('</td>' . "\n", $sortie);
...
if (!$texte) {
	...
	echo ' name="lieux" value="1"/>' . my_html($LG_LPersG_display_places);
	echo '</td>' . "\n";
	...
	if ($est_gestionnaire) {
		...
		echo ' name="manquant" id="manquant" value="1"/>' . my_html($LG_LPersG_display_missing) . ' ';
		echo '<input type="checkbox"';
		if ($omanquant) echo ' checked="checked"';
		echo ' name="omanquant" id="omanquant" value="1"'
			. ' onclick="if (document.getElementById(\'omanquant\').checked==true) {document.getElementById(\'manquant\').checked=true;}"'
			//. ' onclick="document.getElementById(\'manquant\').checked=true;"'
			. '  value="1"/>' . my_html($LG_LPersG_display_only_missing);

		//			document.getElementById('button-checkbox-'+source).checked = false
		//. ' name="omanquant" id="omanquant" value="1"/>'.my_html($LG_LPersG_display_only_missing);
		echo '</td>' . "\n";
	}
