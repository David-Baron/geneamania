...
if ($est_gestionnaire) {
	...
	if ($bt_OK) {
		...
		if ($Sortie != 't') {
			...
			echo '<button type="submit" class="positive">' .
				'<img src="' . $chemin_images . $Icones['chercher'] . '" alt=""/>Nouvelle recherche</button>';
			echo '</div>';
			echo '</form>' . "\n";
		}
	}
	...
	if ((!$bt_OK) && (!$bt_An)) {
		...
		echo '<option value="-">-- Pas de restriction --</option>' . "\n";
		$liste_objets = 'PUFEVDROLS';
		...
		if ($est_privilegie) echo '<input type="radio" name="Sortie" value="c"/> ' . $LG_Ch_Output_CSV;
		echo '</td></tr>' . "\n";
