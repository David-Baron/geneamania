...
function Affiche_Personne($Reference)
{
	...
	if ($Personne = $Res->fetch(PDO::FETCH_NUM)) {
		if (($_SESSION['estPrivilegie']) or ($Personne[3]) == 'O') {
			if ($Sortie == 'e') {
				$Ref_P = $Personne[0];
				echo '<a ' . Ins_Ref_Pers($Ref_P) . '>' .
					my_html($Personne[1] . ' ' . $Personne[2]) . '</a> ';
				...
				echo my_html($Personne[1] . ' ' . $Personne[2]);
			}
		} else {
...
if ($Sortie != 't') {
	...
	if ($res->rowCount() > 0) {
		...
		echo '<input type="radio" name="Sortie" value="e" checked="checked"/> ' . $LG_Ch_Output_Screen . ' ';
		echo '<input type="radio" name="Sortie" value="t"/> ' . $LG_Ch_Output_Text;
		echo '</td></tr>' . "\n";
		...
		if ($Environnement == 'L') {
			...
			echo ' name="sauver" value="save"/> ' . $LG_Yes . '</td></tr>' . "\n";
		}
...
if ($bt_OK) {
	...
	if (!$erreur) {
		...
		if ($Sortie == 'e') {
			$h_deg = ' ' . my_html(LG_CH_RELATED_DEGREE);
			if ($droite or $gauche) {
				echo '<br />' . my_html(LG_CH_RELATED_CANON_LAW) . LG_SEMIC . $gauche . ieme($gauche);
				if ($droite != $gauche) echo ' ' . my_html(LG_CH_RELATED_ON) . ' ' . $droite . ieme($droite);
				echo $h_deg;
				$somme = $gauche + $droite;
				echo '<br />' . my_html(LG_CH_RELATED_CIVIL_LAW) . LG_SEMIC . $somme . ieme($somme);
				echo $h_deg . '<br />';
			}
		}
	}
