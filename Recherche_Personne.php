...
function Ajb_Zone_Req($NomRub,$Rub,$TypRub,&$LaReq,$Zone) {
	...
	if ($Rub != '') {
		...
		if (($NomRub == 'Ville_Naissance') or ($NomRub == 'Ville_Deces'))
			$C_Rub = lib_ville($Rub);
		$le_crit = $C_Rub;
		if (get_magic_quotes_gpc()) $le_crit = stripslashes($le_crit);
		if ($NomRub != 'Categorie') {
		...
		//if (!get_magic_quotes_gpc()) $contenu = addslashes($Rub);
		//else                         $contenu = $Rub;
...
if ($bt_OK) {
	...
	if ($Liste_Requetes == 0) {
		...
		if ($Annee != '') {
			...
			if (!get_magic_quotes_gpc()) $Annee = addslashes($Annee);
		...
		if ($NomP != '') {
			$NomP_SL = $NomP;
			if (get_magic_quotes_gpc()) $NomP_SL = stripslashes($NomP_SL);
		...
		if ($Memo_Req == 'O') {
			...
			if (get_magic_quotes_gpc()) $req_sl = addslashes($req);
			else $req_sl = $req;
			...
			if ($enreg = $res->fetch(PDO::FETCH_NUM)) {
				...
				if ($nb_crit > 0) {
					for ($nb=0; $nb < $nb_crit-1; $nb++) {
						...
						if (get_magic_quotes_gpc()) $le_crit = stripslashes($C_Rub);
						$est_cat = false;
    ...
    if ($Sortie != 't') {
	    ...
	    echo '<br />';
	   	...
	   	echo '<button type="submit" class="positive"><img src="'.$chemin_images.$Icones['chercher'].'" alt=""/>'.$lib_Nouv_Rech.'</button>';
       	if ((!$SiteGratuit) or ($Premium)) {
		   	echo '<button type="submit" onclick="document.forms.nouvelle.reprise.value=\'reprise\'; "'.
		   	 ' class="positive"><img src="'.$chemin_images.$Icones['chercher'].'" alt=""/>'.$lib_Nouv_Rech_Aff.'</button>';
       	}
		echo '</div>';
	    echo '</form>'."\n";
    }
...
if ((!$bt_OK) && (!$bt_An)) {
	...
	if ((!$SiteGratuit) or ($Premium)) {
		...
		if ($est_gestionnaire) {
			...
			$LeMax2 = $LeMax + 1;
			echo '<input type="checkbox" name="Memo_Req" value="O"/>&nbsp;'.my_html(LG_PERS_REQ_REQUEST_TITLE).' <input type="text" size="80" name="TitreReq" value="Requ&ecirc;te '.$LeMax2.'"/>';
			echo '</td></tr>'."\n";
		}
		...
		if ($est_privilegie) {
			if ($LeMax > 0) {
				...
				echo '<option value="0">-- '.my_html(LG_PERS_REQ_REQUEST_CHOOSE).' --</option>'."\n";
	...
	echo '/>';
	echo '<input type="radio" name="Son" value="o"';
	if ($reprise) {
		if ($Son=='o') echo $checked;
	}
	else echo $checked;;
	echo '/>'.my_html(LG_PERS_REQ_SPELL_EXACT).'&nbsp;';
	echo '<input type="radio" name="Son" value="p"';
	if ($reprise) {
		if ($Son=='p') echo $checked;
	}
	echo '/>'.my_html(LG_PERS_REQ_SOUND_EXACT).'&nbsp;';
	echo '<input type="radio" name="Son" value="a"';
	if ($reprise) {
		if ($Son=='a') echo $checked;
	}
	echo '/>'.my_html(LG_PERS_REQ_SOUND_NEAR);
	echo '</td></tr>'."\n";
	...
	echo '<img id="calc1" src="'.$chemin_images.$Icones['calculette'].'" alt="'.$texte_im.'" title="'.$texte_im.'"'.
	...
	echo '/>'."\n";
	echo '<input type="radio" name="TypeAnnee" value="n"';
	if ($reprise) {
		if ($TypeAnnee=='n') echo $checked;
	}
	else echo ' checked="checked"';
	echo '/>'.my_html($LG_birth).'&nbsp;';
    echo '<input type="radio" name="TypeAnnee" value="d"';
   	if ($reprise) {
		if ($TypeAnnee=='d') echo $checked;
	}
    echo '/>'.my_html($LG_death).'&nbsp;';
    $texte_image = LG_PERS_REQ_OFF_DOWN;
	echo '&nbsp;(+/- <img src="'.$chemin_images.$Icones['moins'].'" alt="'.$texte_image.'" title="'.$texte_image.'" border="0" ';
	...
	echo '<img src="'.$chemin_images.$Icones['plus'].'" alt="'.$texte_image.'" title="'.$texte_image.'" border="0" ';
	echo 'onclick="document.forms.saisie.Tolerance.value++;"/> '.my_html(LG_PERS_REQ_OFF_YEARS).')&nbsp;&nbsp;'."\n";
    echo '<input type="radio" name="TypeAnnee" value="v"';
   	if ($reprise) {
		if ($TypeAnnee=='v') echo $checked;
	}
    echo '/>'.my_html(LG_PERS_REQ_ALIVE)."\n";
    echo '</td></tr>'."\n";
    ...
    if ($est_gestionnaire) {
		...
		echo '<input type="radio" name="Diffu" value="O"';
	   	if ($reprise) {
			if ($Diffu=='O') echo $checked;
		}
		echo '/>'.my_html($LG_Yes).'&nbsp;';
		echo '<input type="radio" name="Diffu" value="N"';
	   	if ($reprise) {
		...
		echo '<input type="radio" name="Statut_Fiche" value="O"';
	   	if ($reprise) {
			if ($Statut_Fiche=='O') echo $checked;
		}
		echo '/>'.my_html(LG_CHECKED_RECORD_SHORT).'&nbsp;';
		echo '<input type="radio" name="Statut_Fiche" value="N"';
	   	if ($reprise) {
			if ($Statut_Fiche=='N') echo $checked;
		}
		echo '/>'.my_html(LG_NOCHECKED_RECORD_SHORT).'&nbsp;';
		echo '<input type="radio" name="Statut_Fiche" value="I"';
	   	if ($reprise) {
			if ($Statut_Fiche=='I') echo $checked;
		}
		echo '/>'.my_html(LG_FROM_INTERNET);
		echo '</td></tr>'."\n";
		...
		while ($enr_cat = $res_cat->fetch(PDO::FETCH_NUM)) {
			...
			echo '/><img src="'.$chemin_images.$Icones[$nom_cat].'" border="0" alt="'.$titre_cat.'" title="'.$titre_cat.'"/>'.'&nbsp;&nbsp;'."\n";
		}
		$nb_tag = 0;
		echo '<input type="radio" name="Categorie" value="0"';
		echo '/>'.my_html(LG_PERS_NO_CATEGORY);
		echo '</td></tr>'."\n";
    }
	...
	echo '<input type="radio" name="Tri" value="np"';
    if ($reprise) {
		if ($Tri=='np') echo $checked;
	}
    echo '/>'.my_html(LG_PERS_REQ_SORT_NS).'&nbsp;';
	echo '<input type="radio" name="Tri" value="dn"';
    if ($reprise) {
		if ($Tri=='dn') echo $checked;
	}
    echo '/>'.my_html(LG_PERS_REQ_SORT_BORN).'&nbsp;';
	echo '<input type="radio" name="Tri" value="dd"';
    if ($reprise) {
		if ($Tri=='dd') echo $checked;
	}
    echo '/>'.my_html(LG_PERS_REQ_SORT_DEATH)."\n";
	echo '</td></tr>'."\n";
	
    colonne_titre_tab($LG_Ch_Output_Format);
	echo '<input type="radio" name="Sortie" value="e" checked="checked"/>&nbsp;'.my_html($LG_Ch_Output_Screen).'&nbsp;&nbsp;';
	echo '<input type="radio" name="Sortie" value="t"/>&nbsp;'.my_html($LG_Ch_Output_Text).'&nbsp;&nbsp;';
	// L'export CSV n'est disponible qu'à partir du profil privilégié
	if ($est_privilegie) echo '<input type="radio" name="Sortie" value="c"/>&nbsp;'.my_html($LG_Ch_Output_CSV);
	echo '</td></tr>'."\n";
