...
$titre = $LG_Menu_Title['Town_Search'];     // Titre pour META
$x = Lit_Env();
...
if ($bt_OK) {
	...
	if ($Sortie == 'c') {
		...
		$NomV_fic = $chemin_exports . 'recherche.csv';
		$fp = fopen($NomV_fic, 'w+');
	}
	...
	if ($Departement > -1)
		Ajb_Zone_Req('Zone_Mere', $Departement, 'N', $req, LG_TOWN_SCH_COUNTY);
...
if ((!$bt_OK) && (!$bt_An)) {
	...
	*/
	...
	echo '<td class="value">';
	echo '<input type="radio" name="Sortie" value="e" ' . $checked . '/> ' . my_html($LG_Ch_Output_Screen) . '  ';
