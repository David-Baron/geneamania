...
$sql = 'select count(*), nom, prenoms, idNomFam' . $critere .
	' from ' . $n_personnes .
	' group by nom, prenoms' . $critere .
	' having count(*) > 1';

$echo_modif = Affiche_Icone('fiche_edition', my_html($LG_modify)) . '</a>';
$info = $LG_Menu_Title['Compare_Persons'];
$icone_compare = '<input type="image" src="' . $chemin_images . $Icones['2personnes'] . '" alt="' . $info . '" ' .
		...
		if ($res2 = lect_sql($sql2)) {
			...
			while ($enreg2 = $res2->fetch(PDO::FETCH_ASSOC)) {
				...
				if (!$texte) {
					...
					echo '<input type="radio" name="ref1" value="' . $ref . '"/>';
					echo '<input type="radio" name="ref2" value="' . $ref . '"/>';
					echo '</td>';
				}
				if ($num_pers == 1) {
					if (!$texte) {
