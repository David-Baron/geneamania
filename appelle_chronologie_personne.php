	...
	for ($nb = 0; $nb < count($les_dates); $nb++) {
		...
		if ($la_date <= $maxi) {
			...
			if ($composants[1] == 'AGE10') {
				...
				switch ($composants[1]) {
					case 'N':
						$p_droite = my_html($LG_birth);
						break;
					case 'M':
						$p_droite = my_html($LG_wedding);
						...
						break;
					case 'D':
						$p_droite = my_html($LG_death);
						break;
					case 'DC':
						$p_droite = my_html($LG_death . ' ' . LG_HUSB_WIFE) . ' ' . Lien_vers_Pers();
						break;
					case 'ENN':
						$p_droite = my_html($LG_birth . ' ' . lib_sexe_enfant($composants[6]));
						if (($est_privilegie) or ($composants[4] == 'O')) {
							$p_droite .= ' : ';
							$app_lien = true;
						}
						break;
					case 'END':
						$p_droite = my_html($LG_death . ' ' . lib_sexe_enfant($composants[6]));
						if (($est_privilegie) or ($composants[4] == 'O')) {
							$p_droite .= ' : ';
							$app_lien = true;
						}
						break;
					case 'EVDC':
						$p_droite = my_html($LG_ICSV_Event_Beg . ' ' . $LG_Event . ' ' . $LG_ICSV_Country . ' ' . $composants[2]);
						break;
					case 'EVFC':
						$p_droite = my_html($LG_ICSV_Event_End . ' ' . $LG_Event . ' ' . $LG_ICSV_Country . ' ' . $composants[2]);
						break;
					case 'EVDE':
						$p_droite = my_html($LG_ICSV_Event_Beg . ' ' . $LG_Event . ' ' . $composants[2]);
						break;
					case 'EVFE':
						$p_droite = my_html($LG_ICSV_Event_End . ' ' . $LG_Event . ' ' . $composants[2]);
						break;
					case 'EVDP':
						$p_droite = my_html($LG_ICSV_Part_Beg . ' ' . $composants[2]);
						break;
					case 'EVFP':
						$p_droite = my_html($LG_ICSV_Part_End . ' ' . $composants[2]);
						break;
					default:
