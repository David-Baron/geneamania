...
if ($res = lect_sql($sql)) {
	if ($infos = $res->fetch(PDO::FETCH_NUM)) {
		...
		if (($est_privilegie) or ($infos[4] == 'O')) {
			...
			echo HTML_ou_PDF('<h3 align="center">'.my_html($Prenoms.' '.$Nom).$lib_Ne.$lib_Decede,$sortie);
			if ($approx) echo HTML_ou_PDF('<br />-- '.my_html($LG_ICSV_Event_Ca_Dates).' --',$sortie);
			...
			if (Get_Parents($Refer,$Pere,$Mere,$Rang)) {
				...
				if ($Pere != 0) {
					if (Get_Nom_Prenoms($Pere,$Nom,$Prenoms)) {
						if ($Diff_Internet_P) $lib_P = $lib_P . my_html($LG_of .' ').$Prenoms.' '.$Nom;
					}
				}
				if ($Mere != 0) {
					if (Get_Nom_Prenoms($Mere,$Nom,$Prenoms)) {
						if ($Diff_Internet_P) {
							if ($Pere != 0) $lib_P = $lib_P . my_html(' '.$LG_and.' ');
							$lib_P = $lib_P . my_html($LG_of.' ') .$Prenoms.' '.$Nom;
						}
					}
				}
				if ($lib_P != '') {
					if ($sortie_pdf) echo HTML_ou_PDF('<br />',$sortie);
					echo HTML_ou_PDF('<h3 align="center">'.my_html(ucfirst($lib_fil).' ').$lib_P.'</h3>',$sortie);
				}
			}
	...
	for ($nb=0; $nb<count($les_dates); $nb++) {
		...
		if ($la_date <= $maxi) {
			...
			if ($nb==0) {
				...
				$offset = Age_Mois($date_prec,$la_dateC)/12	;
	...
	for ($nb=0; $nb<count($les_dates); $nb++) {
		...
		if ($la_date <= $maxi) {
			...
			if ($composants[1] == 'AGE10') {
				...
				$app_lien = false;
				switch ($composants[1]) {
					case 'N' : $p_droite = my_html($LG_birth); break;
					case 'M' : $p_droite = my_html($LG_wedding);
								if (($est_privilegie) or ($composants[4] == 'O')) {
									$p_droite .= my_html(' '.$LG_with.' ');
									$app_lien = true;
								}
								break;
					case 'D' : $p_droite = my_html($LG_death); break;
					case 'DC' : $p_droite = my_html($LG_death.' '.LG_HUSB_WIFE) . ' '. Lien_vers_Pers(); break;
					case 'ENN' : $p_droite = my_html($LG_birth.' '.lib_sexe_enfant($composants[6]));
								if (($est_privilegie) or ($composants[4] == 'O')) {
									$p_droite .= ' : ';
									$app_lien = true;
								}
								break;
					case 'END' : $p_droite = my_html($LG_death.' '.lib_sexe_enfant($composants[6]));	
								if (($est_privilegie) or ($composants[4] == 'O')) {
									$p_droite .= ' : ';
									$app_lien = true;
								}
								break;
					case 'EVDC' : $p_droite = my_html($LG_ICSV_Event_Beg.' '.$LG_Event.' '.$LG_ICSV_Country.' '.$composants[2]);
								break;
					case 'EVFC' : $p_droite = my_html($LG_ICSV_Event_End.' '.$LG_Event.' '.$LG_ICSV_Country.' '.$composants[2]);
								break;
					case 'EVDE' : $p_droite = my_html($LG_ICSV_Event_Beg.' '.$LG_Event.' '.$composants[2]);
								break;
					case 'EVFE' : $p_droite = my_html($LG_ICSV_Event_End.' '.$LG_Event.' '.$composants[2]);
								break;
					case 'EVDP' : $p_droite = my_html($LG_ICSV_Part_Beg.' '.$composants[2]);
								break;
					case 'EVFP' : $p_droite = my_html($LG_ICSV_Part_End.' '.$composants[2]);
								break;
