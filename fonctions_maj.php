...
function Ajoute_Nom($idNom, $Nom)
{
	global $connexion;
	if ($idNom == 0) {
		// if (!get_magic_quotes_gpc()) { $Nom = addslashes($Nom); }
		...
		if ($idNom == 0) {
			...
			// if (!get_magic_quotes_gpc()) { $codePho = addslashes($codePho); }
...
{
	global $req_comment;
	// if (($force_slash) or (!get_magic_quotes_gpc())) $DiversP = addslashes($DiversP);
...
{
	global $chemin_images, $Icones, $LG_Add_Event_Mult_Quick, $LG_Add_Event_Quick, $LG_Add_Event, $LG_Del_Event;
	echo '<br /><br />' . "\n";
	if ($Cible == 'P') echo my_html($LG_Add_Event_Mult_Quick) . ' : ' . "\n";
	else echo my_html($LG_Add_Event_Quick) . ' : ' . "\n";
	echo '<img src="' . $chemin_images . $Icones['ajout'] . '" id = "ajout" alt="' . $LG_Add_Event . '" title="' . $LG_Add_Event . '" onclick=\'addRowToTable();\'/>' . "\n";
	echo '<img src="' . $chemin_images . $Icones['supp'] . '" id="delete" alt="' . $LG_Del_Event . '" title="' . $LG_Del_Event . '" onclick=\'removeRowFromTable();\'/> ' . "\n";
...
function ctrl_fichier_ko($nb_fic = -1)
{
	...
	// Contr�le de la taille du fichier par rapport � upload_max_filesize et post_max_size
	if ($nb_fic == -1) {
...
function retourne_var_post($nom_var, $numero)
{
	...
	if (isset($_POST[$concat_nom])) {
		...
		// if (!get_magic_quotes_gpc()) $retour = addslashes($retour);
...
{
	// if (!get_magic_quotes_gpc()) $cont = addslashes($cont);
...
{
	// if (!get_magic_quotes_runtime()) $cont = addslashes($cont);
	$cont = addslashes($cont);
	return $cont;
}
