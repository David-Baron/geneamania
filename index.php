...
if ($NomU != '') {
	...
	if ($debug) echo $NomU . ' / ' . $motPasse . ' / ' . $motPasseSha;
...
if (substr($Lettre_B, strlen($Lettre_B) - 1) != '-') {
	//echo '<img class="img-b" src="'.$chemin_images.$Lettre_B.'" width="45" alt="B"/>';
	echo '<img class="img-b" src="' . $chemin_images . $Lettre_B . '" width="45" alt="B" />';
	$lib = substr($lib, 1);
}
...
if (($vers_fic == $Version) and (!$maintenance)  and (!$verrou)) {
	...
	if (!$Base_Vide) {
		...
		echo '<tr><td><fieldset style="width:90%;">'
			. '<legend>' . my_html($LG_index_quick_search) . ' ' . Affiche_Icone('help', my_html($LG_index_tip_search)) . '</legend>';
		...
		echo '<td rowspan="2" valign="middle"><input type="submit" name="ok" value="' . $lib_Rechercher . '" style="background:url(' . $chemin_images . $Icones['chercher'] . ') no-repeat;padding-left:18px;" /></td></tr>';
		...
		echo '<input type="submit" name="ok" value="' . $lib_Rechercher . '" style="background:url(' . $chemin_images . $Icones['chercher'] . ') no-repeat;padding-left:18px;" />';
		echo '</form>' . "\n";
		echo '</fieldset>';
		echo '</td>';
		echo '</tr>';
	...
	echo '<br /><a href="http://forum.geneamania.net/" target="_blank">' . my_html($LG_index_forum) . '</a>' . "\n";
	echo '<br /><br />' . Affiche_Icone('etoile', $LG_star) . ' <a href="http://www.geneamania.net/demande_site.php" target="_blank">' . my_html($LG_index_ask_site) . '</a> ' . Affiche_Icone('etoile', 'etoile');
	echo '<br /><br /><a href="http://www.geneamania.net/" target="_blank"><b>GENEAMANIA</b></a>, ' . my_html($LG_index_version) . ' ' . $Version . "\n";
	if ($SiteGratuit) {
	...
	if ($vers_fic != $Version) {
		...
		echo '<a href="install.php">' . my_html($LG_index_migrate_here) . '</a>'; // pour migrer votre base.';
	}
}
		...
		if (!$verrou) {
			...
			if ($_SESSION['nomUtilisateur'] == $util_defaut) {
				...
				if ($_SESSION['tentatives'] <= $max_tentatives) {
					...
					echo '<input type="submit" name="ok" value="' . $lib_Connecter . '" style="background:url(' . $chemin_images . $Icones['connecter'] . ') no-repeat;padding-left:18px;"
			...
			// L'utilisateur est connecté
				...
				echo '<input type="submit" name="sortir" value="' . $lib_Deconnecter . '" style="background:url(' . $chemin_images . $Icones['deconnecter'] . ') no-repeat;padding-left:18px;" />';
				echo '</td></tr>';
			}
		}
		// Site verrouillé
