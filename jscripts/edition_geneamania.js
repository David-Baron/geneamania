...
// Ajout d'un texte � la fin d'un TextArea
// Param�tres : le texte et le nom du TextArea
...
// Sur le clic sur l'image URL, on affiche d�but ou fin de balise dans le TextArea
// Le nom de la zone debUrl est fixe (un seul TextArea autoris� pour le moment...)
...
// On �crit le bloc d�but fin d'un coup...
function Aj_URL2(nom_TxtArea) {
    AjoutTxt('[URL]Ins�rez l\'adresse ici...[/URL]',nom_TxtArea);
}

// Affichage d'un message de confirmation sur une demande de suppression
function confirmer(quoi,nom_btsup) {
	//if(confirm("Etes-vous s�r(e) de vouloir supprimer "+quoi+" ?")) return true;
	//else {
	//  nom_btsup.value = '-';
	//  return false;
	//}
	if(confirm("Etes-vous s�r(e) de vouloir supprimer "+quoi+" ?")) {
...
//ouverture popup centr�e de saisie de date
...
// V�rification de la pr�sence d'une zone
...
// V�rification des zones obligatoires seulement sur le bouton ok
function verification_form(formulaire,zones) {
	...
	if (formulaire.cache.value == 'ok') {
		...
		if (absentes > 0) {	
			window.alert('Pensez � remplir les champs obligatoires...');
			retour = false;
		}
	}
	return retour;
...
// Contr�le de num�ricit� d'une zone
function verification_num(zone) {
	var ko = isNaN(zone.value);
	if (ko) {	
		window.alert('Attention, zone non num�rique...');
		zone.value = 0;
	}
}
...
// D�claration d'un objet pour Ajax
